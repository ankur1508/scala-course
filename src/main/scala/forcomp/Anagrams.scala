...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (for(ch <- w.toLowerCase) yield (ch, 1)).groupBy(_._1)
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary.map(word => (wordOccurrences(word), word))
    .groupBy(_._1)
    .map(entry => (entry._1, for(x <- entry._2) yield x._2))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences match {
      case Nil => List(List())
      case (x :: xs) => {
        val c : List[Occurrences] = (x match {
          case (ch, count) => for(i <- 1 to count) yield List((ch, i))
        }).toList
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val xMap = x.toMap
    val yMap = y.toMap
    xMap.map{case (k, v) => if(yMap.contains(k)) (k, v - yMap(k)) else (k, v)}
      .filter(tup => tup._2 > 0).toList
